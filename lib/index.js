...
var createForm = function(pane) {
  ...
  pane.formSpec.forEach(function(item) {
    if ('select' === item[0]) {
      html += makeSelect(item[1], item[2], item[3], item[4], item[5])
    } else if ('checkbox' === item[0]) {
...
var makeTabPanel = function(pane, paneBody) {
  ...
  + pane.paneId + '">' + '<h4>' + pane.title + '</h4>' + paneBody + '</div>'
};

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
var makeSelect = function(idBase, label, labelCols, selectCols, items) {
  var ret =
  `<label class="col-sm-${labelCols} control-label" for="${idBase}">${label}</label>
  <div class="col-sm-${selectCols}">
