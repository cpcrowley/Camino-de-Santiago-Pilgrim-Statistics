...
// It failed to clear in some cases so do it again 8 seconds later
setTimeout(function() {$('#loading-warning').remove()}, 8000)
...
var recomputePane = function(pane) {
  ...
  if (pane.html) {
    ...
    if (G.pData) {
      ...
      G.getDataPromise.done(function() {
...
//------------------------------------------------------------------------------
//     pane.link = $(`<a href="${pane.paneId}" role="tab">
//    <span class="cc-tab-title">${pane.title}</span></a>`)
...
var addPanesToTabMenu = function() {
  ...
  paneDefinitions.forEach(function(pane, index) {
    ...
    paneIndex += 1
    pane.link = $('<a href="#' + pane.paneId + '" role="tab"><b>' + pane.title + '</b></a>')
    .on('click', function() {
      if (!pane.form) {
