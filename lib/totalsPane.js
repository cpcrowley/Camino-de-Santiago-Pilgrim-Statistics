...
var recomputePane = function() {
  ...
  var tag = G.mainSelect.val()
  //console.log('tag='+tag, G.mainSelect[0])

  if (tag === 'about') {
    G.mainContent.append(about())
    return

  } else if (tag === 'welcome') {
    G.mainContent.append(welcome())
    return

  } else if (tag === 'notes') {
    G.mainContent.append(notes())
    return
  }
  ...
  if (currentOption.showMinPilgrimsSelect) {
    var m = G.minPilgrimsInCol
    resultsDiv.append(makeSelect('min-pilgrims-in-col', 'Columns with >= N pilgrims in 2004-2016', [
      ...
      G.minPilgrimsInCol = parseInt(G.mainContent.find('.min-pilgrims-in-col').val())
      recomputePane()
    }))
  ...
  G.mainContent.append(resultsDiv)
