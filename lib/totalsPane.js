...
var makeTotalsHTML = (params) => {
  ...
  if (!isMonthOption) {
    ...
    labels = _.filter(labels, label => {
      var p = _.find(optionsField, p => p[0]===label)
      if (!p) {
  ...
  _.forEach(labels, (label) => { html += `<th>${addNBSPtoY(removeDashs(label))}</th>` })
  ...
  if (currentOption.tag === 'by-country') {
    ...
    _.forEach(labels, (label) => {
      ...
      html += `<th>${addNBSPtoAmpersand(englishLabel)}</th>`
    })
    html += '</tr>'
  }
  ...
  switch (currentOption.tag) {
    ...
    Oikoten: See <a src="https://newint.org/features/2012/07/01/redemption-road-pilgrimage/">here</a>
</div>`
    break
