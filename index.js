...
var G = {}
G.pData = null
console.log('Starting pData fetch')
G.getDataPromise = $.ajax({
  dataType: "json",
  url: './data/pData.json',
  success: data => {
    console.log('Fetch of  pData complete', data)
    G.pData = data
...
var paneDefinitions = [
  {
    ...
    formSpec: [
      ...
      ['select', 'max-columns', 'Maximum columns', [
...
var recomputePane = function(pane) {
  ...
  if (pane.html) {
    ...
    if (G.pData) {
      ...
      console.log('***** pData not ready. Wait until it is ready.')
      G.getDataPromise.done(()=>{
        console.log('CALLED G.getDataPromise.done')
        recomputePane(paneDefinitions[0])
      })
    }
  } else {
